...
type Action =
  ...
  | { type: "SET_PRIMARY_PAYMENT_OPTION"; payload: any };
...
export const ProfileProvider: React.FunctionComponent<ProfileProviderProps> = ({
  ...
  const getPrimaryAddress = () => {
    return state.address.filter((address) => address.type === "primary")[0];
  };

  const getPrimaryPaymentOption = () => {
    return state.selectedPaymentOption;
  };

  const profileGetOperations = { getPrimaryAddress, getPrimaryPaymentOption };

  return (
    <ProfileContext.Provider value={{ state, dispatch, profileGetOperations }}>
      {children}
